---
title: "Statistical Inference for Numerical Data"
...
```{r habit-weight-box,warning=FALSE}
# type your code for the Question 3 here, and Knit
fig <- plot_ly(y=~nc$weight,x=~nc$habit,type = "box",color=~nc$habit) %>%
  layout(xaxis=list(title="Mother"),yaxis=list(title="Infant Weight (lbs)"))
fig
```
...
```{r gained-mature-boxplot,warning=FALSE}
# type your code for the Exercise here, and Knit
fig <- plot_ly(y=~nc$gained,x=~nc$mature,type = "box",color=~nc$mature) %>%
  layout(xaxis=list(title="Mother"),yaxis=list(title="Weight Gained (lbs)"))
fig
```

```{r gained-mature-ht,warning=FALSE}
...
nc %>% group_by(mature) %>%   summarise(mu = mean(mage), pop_med = median(mage), 
            sigma = sd(mage), pop_iqr = IQR(mage),
            pop_min = min(mage), pop_max = max(mage),
            pop_q1 = quantile(mage, 0.25),  # first quartile, 25th percentile
            pop_q3 = quantile(mage, 0.75))  # third quartile, 75th percentile
```
